...
| IMP.4 | Implement a heartbeat mechanism. Add to unit tests in the WebSocketLibrary.Tests project. Heartbeat should: send periodic ping framtes to clients (using WebSocket protocol built-in ping/pong mechanism); Track which clients respond with pong frames; Disconnect clients that fail to respond to multiple pings | Pending |
